...
<img src="semgrep03-底层技术原理.assets/image-20260802222608006.png" alt="image-20260802222608006" style="zoom: 50%;" />
...
- 源码解析的输入是完整的目标程序，输出要支持后续命名解析和 IL/CFG 构建，服务于数据流分析
- 规则解析的输入是 YAML/JSON 这种"配置数据"，绝大部分字段解析只是字典取值和结构校验，只有 `pattern:` 相关字符串会"借用"源码解析器的前半段（CST→语言AST→AST_generic），但止步于此，产出的是可被结构化匹配（`Pattern_vs_code.ml`）直接使用的 pattern AST，不需要也不会做命名消解或 IL 化
